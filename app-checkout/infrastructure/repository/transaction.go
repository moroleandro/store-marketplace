...
import (
	"database/sql"
	"errors"
	"github.com/moroleandro/marketplace-checkout/app-checkout/domain"
)
...
func (t *TransactionRepositoryDb) CreateCreditCard(creditCard domain.CreditCard) error {
	stmt, err := t.db.Prepare(`insert into credit_cards(id, name, number, expiration_month, expiration_year, CVV, balance, limit_card) 
...
func (t *TransactionRepositoryDb) GetCreditCard(creditCard domain.CreditCard) (domain.CreditCard, error) {
	...
	stmt, err := t.db.Prepare("select id, balance, limit_card from credit_cards where number=$1")
	if err != nil {
		return c, err
	}
	if err = stmt.QueryRow(creditCard.Number).Scan(&c.ID, &c.Balance, &c.LimitCard); err != nil {
